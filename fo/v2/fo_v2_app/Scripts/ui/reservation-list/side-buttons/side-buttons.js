...
function setUrlToRelevetReservation(e) {
    ...
    IsAttachToAllotment = $('input[class="data-reservation-no"]:checked').attr('data-IsAttachToAllotment');

    membercode = $('input[class="data-reservation-no"]:checked').attr('data-member-code');
    ...
    $('#btnInvoiceDetail').attr('href', '/Reporting/Report/ViewReport?ReportName=ProformaInvoiceDetail' + '&Uuid=' + resUuid + '&PropertyId=' + propertyId);
    $('#btnInvoiceSummary').attr('href', '/Reporting/Report/ViewReport?ReportName=ProformaInvoiceSummary' + '&Uuid=' + resUuid + '&PropertyId=' + propertyId);
    $('#btnInvoiceDetailBc').attr('href', '/Reporting/Report/ViewReport?ReportName=ProformaInvoiceDetailBC' + '&Uuid=' + resUuid + '&PropertyId=' + propertyId);
    $('#btnInvoiceSummaryBc').attr('href', '/Reporting/Report/ViewReport?ReportName=ProformaInvoiceSummaryBC' + '&Uuid=' + resUuid + '&PropertyId=' + propertyId);
...
$('body').off('click', '#single-edit-reservation').on('click', '#single-edit-reservation', function (e) {
    ...
    var url = '/Reservation/ReservationList/SinglePageReservationEdit?guestName=' + guestName +
        '&departureDate=' + departureDate +
        '&arrivalDate=' + arrivalDate +
        '&roomCode=' + roomCode +
        '&resheaderId=' + resheaderId +
        '&roomCategoryId=' + roomCategoryId +
        '&rateCodeHeaderId=' + rateCodeHeaderId +
        '&mealPlanId=' + mealPlanId +
        '&roomTypeId=' + roomTypeId +
        '&isLocalRate=' + isLocalRate +
        '&isChangeRoom=1&type=' + reservationType +
        '&isDormitory=' + isDormitory +
        '&groupReservationNo=' + groupResNo +
        '&Uuid=' + resUuid;

    var title = "Change Reservation Details " + resNo;

    // Check allotment attachment
 
    if (IsAttachToAllotment == "true") {

        var confirmationMessage =
            "This reservation is currently attached to an allotment.\n\n" +
            "If you change the stay period or allocated room category, " +
            "the system will automatically remove the attached allotment.\n\n" +
...
$('body').off('click', '#AttachToMember').on('click', '#AttachToMember', function (e) {
    e.preventDefault();
  
    var url = '/reservation/reservations/MemberSelectionPopup?reservationheaderId=' + resheaderId + '&membercode=' + membercode;

    var title = 'Attache to member ' + (typeof resNo !== 'undefined' ? resNo : '');
    openFullContent(title, url);
